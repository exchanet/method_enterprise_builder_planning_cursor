...
> **System nature (v2.0.0):** This is a **hybrid framework** — the core is a structured prompt system interpreted by the Cursor AI agent, complemented by optional standalone executable tools (adr-validator, microtask-linter, CI/CD scripts) that can be run independently or integrated into any CI/CD pipeline. Hooks and pack behaviors are interpreted by the agent, not executed by a runtime.
