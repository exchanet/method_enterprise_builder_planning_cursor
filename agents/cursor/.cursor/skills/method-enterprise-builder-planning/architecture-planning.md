...
## ADR Validation Step (v2.0.0+)

Before marking any ADR as **Accepted**, run the enterprise validator:
...
# Validate all ADRs in the project
node packs/enterprise-architecture-pack/validators/adr-validator/src/index.ts \
  --strict docs/adr/
...
**Gate:** An ADR that fails any `error`-severity rule cannot be marked as `Accepted`.
Fix all errors, re-run the validator, and confirm `ALL PASSED` before proceeding to Phase 6.

Rules checked: ADR-STR-001/002, ADR-ENT-001..005, ADR-COMP-001..003
See `packs/enterprise-architecture-pack/validators/adr-validator/README.md` for rule details.
